...
static SGS_Result *run_program(SGS_Program *program) {
}
...
SGS_Result *SGS_interpreter_run(SGS_Interpreter *o, SGS_Program *program) {
	SGS_Result *result = run_program(program);
	if (result == NULL) {
		return NULL;
	}
...
void SGS_interpreter_get_results(SGS_Interpreter *o, SGS_Result ***results,
		size_t *count) {
	*results = (SGS_Result**) SGS_PList_ITEMS(&o->results);
	*count = o->results.count;
}
