...
typedef struct EventNode {
	uint32_t waittime;
	uint32_t params;
	SGSResultVoiceData_t voice;
	SGSResultOperatorData_t operator;
} EventNode;
...
SGSRenderer_t SGS_create_renderer(SGSResult_t res, uint32_t srate) {
	 ...
	 */
	size_t size = sizeof(struct SGSRenderer) - sizeof(OperatorNode);
	...
	for (i = 0; i < res->event_count; ++i) {
		...
		if (res_e->voice_data) {
			SGSResultVoiceData_t vd = res_e->voice_data;
 ...
 */
static void SGS_renderer_handle_event(SGSRenderer_t o, EventNode *e) {
	if (1) /* more types to be added in the future */ {
		VoiceNode *vn;
		SGSResultVoiceData_t vd = e->voice;
		OperatorNode *on;
		SGSResultOperatorData_t od = e->operator;
		/*
 ...
 */
static uint32_t run_block(SGSRenderer_t o, uint32_t len, OperatorNode *n,
		uint32_t acc_ind) {
	uint32_t i;
 ...
 */
static uint32_t run_voice(SGSRenderer_t o, VoiceNode *vn, int16_t *out,
		uint32_t buf_len) {
	BufData *input_block;
