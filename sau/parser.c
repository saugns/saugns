...
static void end_operator(sauParser *restrict o) {
	...
	sauScriptOpData *pop = op->ref.prev;
	if (!pop) {
		/*
...
static void begin_list(sauParser *restrict o,
	...
	if (plist != NULL) {
		...
		list->ref.info->type = SAU_POBJT_LIST;
	}
...
static void begin_operator(sauParser *restrict o,
	...
	if (pop != NULL) {
		...
		op->ref.info = sau_mpalloc(o->mp, sizeof(sauScriptObjInfo));
		op->ref.info->first_event = e;
		op->ref.info->type = type;
		if (type == SAU_POBJT_RASG)
			op->ref.info->seed = sau_rand32(&o->sl.math_state);
	}
...
static void leave_level(sauParser *restrict o) {
	...
	if (pl->scope == SCOPE_BIND) {
		...
		pl->parent->set_var = NULL; // may not be used after
		/*
		 * Pop script options.
		 */
...
static bool parse_ev_mode(sauParser *restrict o) {
	...
	sauScanner *sc = o->sc;
	sauScriptOpData *op = pl->operator;
	if (op->ref.info->type != SAU_POBJT_RASG)
		return true; // reject
...
"ignoring 'ti' (implicit time) before ';' without number");
	...
	case 'f':
		...
		break;
	case 'l': {
		if (op->ref.info->type != SAU_POBJT_RASG) goto DEFER;
		size_t id;
		if (!scan_sym_id(sc, &id, SAU_SYM_LINE_ID,
					sauLine_names))
			break;
...
"ignoring 'ti' (implicit time) for non-nested operator");
		...
		break; }
	case 'w': {
		if (op->ref.info->type != SAU_POBJT_WAVE) goto DEFER;
		size_t id;
...
"reference '@%s' doesn't point to an object", var->sstr->key);
		...
		case 'R': {
			...
			begin_operator(o, NULL, false, SAU_POBJT_RASG);
		...
		case 'W': {
			...
			begin_operator(o, NULL, false, SAU_POBJT_WAVE);
			pl.operator->wave = id;
			pl.sub_f = parse_in_opdata;
			break; }
		case '[':
...
/*
 * Post-parse code (currently ran after each durgroup for those nodes)
 */
...
sauVoAlloc_get_id(sauVoAlloc *restrict va,
	...
	for (size_t id = 0; id < va->count; ++id) {
		...
		if (vas->duration_ms == 0) {
			...
			if (old_e->obj_first_ev != NULL)
				old_e = old_e->obj_first_ev;
...
static void time_durgroup_object(sauScriptEvData *restrict e,
	...
	default: {
		...
		if (!(op->op_flags & SAU_SDOP_NESTED)) {
			e->carr_info = obj->info;
//			vas->cur_obj = e->carr_info;
		}
		break; }
	}
}
...
static uint32_t time_operator(sauScriptOpData *restrict op) {
	...
	if (!(op->params & SAU_POPP_TIME))
		op->ref.event->ev_flags &= ~SAU_SDEV_VOICE_SET_DUR;
	if (!(op->time.flags & SAU_TIMEP_SET)) {
		...
		op->ref.event->ev_flags |= SAU_SDEV_LOCK_DUR_SCOPE;
	}
...
static uint32_t time_event(sauScriptEvData *restrict e) {
	...
	while (fork != NULL) {
		...
		sauScriptOpData *ne_op = ne->root_obj,
				 *ne_op_prev = ne_op->ref.prev,
				 *e_op = ne_op_prev;
