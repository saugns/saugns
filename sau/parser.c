...
#define MUSKEY(note, notemod) (((note) * 7) + 3 + (notemod))
#define MUSNOTE(key) ((key) / 7)
...
#endif
	static const float notes_all[3][8] = {
		{	/* Equal temperament */
			1.f,                    // 0	C
...
//			1.f,
	...
	const float *notes = notes_all[sl->sopt.key_system];
	const float *notemods = notemods_all[sl->sopt.key_system];
	...
	case 'd': notemod = -1; freq /= notemods[1]; break; // half-flat
	case 'z': notemod = +1; freq *= notemods[1]; break; // half-sharp
	case 'v': notemod = -3; freq /= notemods[2]; break; // flat-and-a-half
	case 'k': notemod = +3; freq *= notemods[2]; break; // sharp-and-a-half
#if 0
	case 'w': notemod = -4; freq /= notemods[3]; break; // double-flat
	case 'x': notemod = +4; freq *= notemods[3]; break; // double-sharp
#endif
...
typedef struct sauParser {
	...
	sauScriptEvData *events, *last_event;
	sauScriptDurGroup *groups, *cur_group;
} sauParser;
...
static void end_event(sauParser *restrict o) {
	...
	pl->event = NULL;
	sauScriptEvData *group_e = (pl->main_ev != NULL) ? pl->main_ev : e;
...
static bool new_durgroup(sauParser *restrict o) {
	...
	sauScriptDurGroup *group = sau_mpalloc(o->tmp_mp, sizeof(*group));
	if (!group)
		return false;
	if (!o->groups)
		o->groups = group;
...
static void time_durgroup(sauScriptDurGroup *restrict g);
...
static void leave_level(sauParser *restrict o) {
	...
	if (!pl->parent) {
		 ...
		 */
		end_event(o);
		for (sauScriptDurGroup *g = o->groups;
...
"\twith or without added 'b'/'d'/'v' (flat) or 's'/'z'/'k' (sharp)");
			break;
		}
		int sufc, notemod = 0;
