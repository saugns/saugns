...
static void prepare_event(sauParser *restrict o,
	...
	if (!pl->event || pl->add_wait_ms > 0 ||
			((prev_obj || !(pl->parent && pl->parent->nest.list))
			 && pl->event->main_obj) ||
			is_compstep)
...
static void link_ev_obj(sauParser *restrict o,
	...
	struct ParseLevel *pl = o->cur_pl;
	sauScriptEvData *e = pl->event;
	sauScriptListData *nest_list =
		pl->parent ? pl->parent->nest.list : NULL;
	...
	if (prev || !nest_list) {
		...
		if (!nest_list->first_item)
			nest_list->first_item = obj;
		else
			pl->parent->nest.last_obj->next = obj;
		pl->parent->nest.last_obj = obj;
		/* needed for nested list placement, as in "[[] ...]" */
		if (pl->parent->parent &&
		    pl->parent->parent->nest.list == nest_list)
			pl->parent->parent->nest.last_obj = obj;
	}
	if (!pl->scope_first)
		pl->scope_first = obj;
	/*
...
/*
 * Create a new list inside the parent ParseLevel for use in this one.
 */
static void begin_list(sauParser *restrict o,
	...
	parent_pl->nest.list = list;
	if (use_type != SAU_POP_N_carr) {
		sauScriptOpData *parent_on = parent_pl->operator;
		if (!parent_on->mods)
			parent_on->mods = list;
		else
			parent_pl->nest.last_mods->ref.next = list;
		parent_pl->nest.last_mods = list;
...
static void begin_operator(sauParser *restrict o,
	...
	pl->nest.last_mods = NULL; /* now track for this node */
	if (!is_compstep)
		pl->pl_flags |= PL_NEW_EVENT_FORK;
	...
	if (pop != NULL) {
		...
		sauScriptListData *nest_list =
			pl->parent ? pl->parent->nest.list : NULL;
		info->parent_op_obj = nest_list ?
			o->obj_arr.a[nest_list->ref.obj_id].parent_op_obj :
			op->ref.obj_id;
...
static void parse_in_par_sweep(sauParser *restrict o) {
	PARSE_IN__HEAD(parse_in_par_sweep, true)
		sauLine *line = pl->op_sweep;
		double val;
		switch (c) {
		case 'g':
			if (scan_num(sc, pl->numconst_f, &val)) {
				line->vt = val;
		...
		case 't':
			...
			break;
		case 'v':
			scan_line_state(sc, pl->numconst_f, line,
					pl->num_ratio);
			break;
		default:
			goto DEFER;
...
static void parse_op(sauParser *restrict o, uint8_t op_type,
	...
	if (pl->parent && !pl->use_type && pl->parent->op_sweep) {
...
"ignoring label assignment to label assignment");
		...
		case '/':
			if (pl.parent && pl.parent->nest.list) goto INVALID;
			parse_waittime(o);
			break;
		case '<':
