...
static sauScriptObjInfo *ObjInfoArr_add(ObjInfoArr *restrict o,
	...
	sauScriptObjInfo *info = _ObjInfoArr_add(o, NULL);
	if (!info)
		return NULL;
	ref->obj_id = count;
 ...
 */
static void link_ev_obj(sauParser *restrict o,
...
static void begin_list(sauParser *restrict o,
	...
	if (use_type != SAU_POP_CARR) {
		...
		if (!parent_on->mods)
			...
			parent_pl->nest.last_list->ref.next = list;
...
static void begin_operator(sauParser *restrict o,
	...
	if (pop != NULL) {
		...
		info->parent_op_obj = pl->nest.list ?
	...
	if (parent_pl != NULL) {
		...
		if (newscope == SCOPE_NEST) {
			pl->sub_f = (use_type != SAU_POP_DEFAULT
				...
				: NULL;
			pl->set_label = parent_pl->set_label; // for list assign
...
static void leave_level(sauParser *restrict o) {
	...
	if (pl->scope == SCOPE_BIND) {
		...
		if (pl->scope_first != NULL) {
			...
			begin_operator(o, (void*)pl->scope_first, false, 0);
		}
	} else if (pl->scope == SCOPE_NEST) {
		pl->parent->set_label = NULL; // may not be used after
		/*
		 * Pop script options.
		 */
...
static bool parse_so_amp(sauParser *restrict o) {
	...
	if (scan_num(sc, NULL, &val)) {
		...
		if (pl->use_type == SAU_POP_AMOD)
			val *= pl->parent->sopt_save.def_ampmult;
...
static void parse_ev_newop(sauParser *restrict o, uint8_t op_type,
...
static bool parse_ev_modemain(sauParser *restrict o, uint8_t op_type,
	uint8_t sym_type, const char *const* restrict sym_names) {
	struct ParseLevel *pl = o->cur_pl;
	sauScriptOpData *op = pl->operator;
...
"ignoring 'ti' (implicit time) before ';' without number");
	...
	case 'f':
		...
		break;
	case 'l':
		if (parse_ev_modemain(o, SAU_POPT_N_raseg,
				SAU_SYM_LINE_ID, sauLine_names)) goto DEFER;
	...
	case 'm':
		...
		break;
	case 'n':
		if (parse_ev_modemain(o, SAU_POPT_N_noise,
				SAU_SYM_NOISE_ID, sauNoise_names)) goto DEFER;
		break;
	case 'p':
	...
	case 't': {
		...
		switch (suffc) {
		case 'd':
			op->time = sauTime_DEFAULT(o->sl.sopt.def_time_ms, 0);
			break;
		case 'i':
			if (!(op->op_flags & SAU_SDOP_NESTED)) {
				sauScanner_warning(sc, NULL,
"ignoring 'ti' (implicit time) for non-nested operator");
			...
			op->time = sauTime_VALUE(o->sl.sopt.def_time_ms, 1);
			break;
		default:
			if (suffc)
				sauScanner_ungetc(sc);
		...
		break; }
	case 'w':
		if (parse_ev_modemain(o, SAU_POPT_N_wave,
				SAU_SYM_WAVE_ID, sauWave_names)) goto DEFER;
		break;
	default:
		goto DEFER;
	}
...
static bool parse_numvar_lhs(sauParser *restrict o) {
	bool check_unset = sauScanner_tryc(o->sc, '?');
...
"\ttry passing it to the script as an option, \"%s=...\"",
	...
	if (mark_fail) {
		o->script_fail = true;
		o->sc->s_flags |= SAU_SCAN_S_QUIET; // silence normal warnings
	}
...
"label '@%s' doesn't refer to any object", label->sstr->key);
			...
			break; }
		case 'N':
			parse_ev_newop(o, SAU_POPT_N_noise,
					SAU_SYM_NOISE_ID, sauNoise_names);
			break;
		case 'R':
			parse_ev_newop(o, SAU_POPT_N_raseg,
		...
		case 'O':
			...
			/* fall-through */
		case 'W':
			parse_ev_newop(o, SAU_POPT_N_wave,
