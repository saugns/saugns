...
#define CASES_4MODS(ID, FIELD) \
	case SAU_POP_N_##ID:     n->FIELD.mods_add	= ids->a; break; \
	case SAU_POP_N_##ID##1:  n->FIELD.mods1 	= ids->a; break; \
	case SAU_POP_N_##ID##2:  n->FIELD.mods2 	= ids->a; break; \
	case SAU_POP_N_##ID##_r: n->FIELD.r_mods	= ids->a; break; \
/**/
	switch (ids->use) {
	case SAU_POP_N_carr:     break;
	CASES_4MODS(   camod,    gen.pan)
	CASES_4MODS(   amod,     gen.amp)
	CASES_4MODS(   fmod,     osc.freq)
	case SAU_POP_N_pmod:     n->osc.pmods    	= ids->a; break;
	CASES_4MODS(   apmod,    osc.pm_a)
...
static void run_block_wosc(sauGenerator *restrict o,
	 ...
	 */
	if (!run_param_with_rangemod(o, bufs, len, &n->osc.freq, parent_freq,
				NULL, true))
		sau_nzerof(*bufs, len);
