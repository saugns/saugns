...
static void update_gen(sauGenerator *restrict o,
	...
	if (valr) for (uint32_t i = 0; i < SAU_PVALR_TYPES; ++i)
...
static void block_mix_add(float *restrict buf, size_t len,
	...
	if (amp) sau_nmulnf(in_buf, len, amp);
	else     sau_nmulf(in_buf, len, amp_v0);
...
/**/
	...
	if (amp) MIX(in_buf[i], amp[i])
	else     MIX(in_buf[i], amp_v0)
 ...
 * instructions for a generator. The exception is, for a root/voice generator,
 * call run_rins_mix_to_voice() after to truly output this signal.
 */
static uint32_t run_rins_gen_pop_mix(struct sauGenerator *restrict o,
	...
	// if samples skipped due to generator time limit, zero the rest
	uint32_t skip_len = o->cur_block->skip_len;
	if (!layer && skip_len > 0) sau_nsetf(out_buf+len, skip_len, 0.0);
...
/*
 * Add generator result as voice output. Must be called directly after
 * run_rins_gen_pop_mix(). The use of the stack is adapted for this.
 */
static uint32_t run_rins_mix_to_voice(struct sauGenerator *restrict o,
		const sauRIns *restrict ins, uint32_t len) {
//	AnyGen *n = (o->cur_block+1)->n;
	float *s_buf = o->bufs[ins->a.i];
	float *pan_buf = !ins->has_b_f ? o->bufs[ins->b.i] : NULL;
	float pan_v0 = ins->b.f; // fallback only
	// needs length before popping time scope
	uint32_t mix_len = (o->cur_block+1)->run_len;
	mix_add(o, s_buf, pan_buf, pan_v0, mix_len);
	return len;
}
...
static uint32_t run_rins_run_par_line(struct sauGenerator *restrict o,
	...
	uint8_t line_type  = sau_rins_get_b2(ins->c.i);
	uint8_t line_flags = sau_rins_get_b3(ins->c.i);
