...
sauVoAlloc_update(sauVoAlloc *restrict va,
	...
	vo_id = va->count;
	if (!(vas = _sauVoAlloc_add(va, NULL)))
		return NULL;
RECYCLED:
...
sauOpAlloc_update(struct ParseConv *restrict o,
		const sauScriptOpData *restrict od) {
	sauScriptObjInfo *info = &o->objects[od->ref.obj_id];
	if (!od->prev_ref) {
		uint32_t op_id = o->oa.count;
		sauOpAllocState *oas = _sauOpAlloc_add(&o->oa, NULL);
		if (!oas)
			return NULL;
...
ParseConv_convert_ops(ParseConv *restrict o,
	...
	if (op_list) for (sauScriptOpData *op = op_list->first_item;
		...
		if (!(info = sauOpAlloc_update(o, op)))
			return false;
		for (sauScriptListData *in_list = op->mods;
				in_list != NULL; in_list = in_list->ref.next) {
