...
typedef struct SemGenObj {
	...
	bool is_dead      : 1; // already marked for reuse (no other uses)
...
typedef struct ParseSem {
	...
	IDBuf gen_obj_dead; // track never-used-again generators for ID reuse
...
sem_gen_obj_add(ParseSem *restrict o, sauParseObjRef *restrict ref,
	...
	uint32_t id;
	SemGenObj *info;
	if (o->gen_obj_dead.count > 0) {
		id = *IDBuf_pop(&o->gen_obj_dead);
		info = &o->gen_obj.a[id];
		*info = (SemGenObj){0};
	} else {
		id = o->gen_obj.count;
...
sem_voalloc_timing(ParseSem *restrict o, sauParseEvData *restrict e) {
	...
	for (uint32_t id = 0; id < o->gen_obj.count; ++id) {
		...
		if (info->is_dead) continue; // pointless to adjust
		if (info->time_ms <= e->wait_ms)
			info->time_ms = 0;
		else
			info->time_ms -= e->wait_ms;
		if (info->has_next_ref) continue;
...
sem_voalloc_update(ParseSem *restrict o, sauParseEvData *restrict e) {
	...
	info = &o->gen_obj.a[(obj_id = info->root_gen_obj)];
	sauVoAllocState *vas;
...
sem_genalloc_update(ParseSem *restrict o, sauParseGenData *restrict g,
	...
	/*
	 * Reuse first lowest free generators if any. Note that when
	 * recursing in deep cloning traversal, reuse is restricted.
	 */
	for (uint32_t id = 0; id < o->gen_obj.count; ++id) {
		SemGenObj *old = &o->gen_obj.a[id];
		if (id != obj_id && old->is_expired && !old->is_dead) {
			if (old->is_labeled || old->is_visited)
				continue; // blocked, unsafe to reuse
			// Mark object ID slot for reuse in next allocation(s).
			old->is_dead = true;
			if (!IDBuf_push(&o->gen_obj_dead, &id))
				return NULL;
			break;
		}
	}
...
sem_handle_event(ParseSem *restrict o, sauParseEvData *restrict e,
	...
	if (vas->has_new_graph || vas->has_gen_expiry) {
