...
static int32_t read_inum(struct SGSReader *fr) {
  char c;
  int32_t num = -1;
  c = SGS_READER_GETC(fr);
  if (c >= '0' && c <= '9') {
    num = c - '0';
    for (;;) {
      c = SGS_READER_GETC(fr);
  ...
  SGS_READER_UNGETC(fr);
  return num;
}

static int32_t read_strfind(struct SGSReader *fr, const char *const*str) {
  int32_t search, ret;
  ...
  free(s);
  SGS_READER_UNGETN(fr, (pos-matchpos));
  return ret;
}

static void read_skipws(struct SGSReader *fr) {
  char c;
  while ((c = SGS_READER_GETC(fr)) == ' ' || c == '\t') ;
  SGS_READER_UNGETC(fr);
}
...
#define NEWLINE '\n'
static char scan_char(SGSParser *o) {
  char c;
  read_skipws(&o->fr);
  if (o->nextc) {
    c = o->nextc;
  ...
  if (c == '#')
    while ((c = SGS_READER_GETC(&o->fr)) != '\n' && c != '\r' && c != 0) ;
  if (c == '\n') {
    SGS_READER_TESTCGET(&o->fr, '\r');
    c = NEWLINE;
  } else if (c == '\r') {
    SGS_READER_TESTCGET(&o->fr, '\n');
    c = NEWLINE;
  } else {
    read_skipws(&o->fr);
  }
...
static void scan_ws(SGSParser *o) {
  char c;
  do {
    c = SGS_READER_GETC(&o->fr);
    if (c == ' ' || c == '\t')
...
static float scan_num_r(SGSParser *o, float (*scan_symbol)(SGSParser *o),
...
static bool scan_num(SGSParser *o, float (*scan_symbol)(SGSParser *o),
                      float *var) {
  char buf[64];
...
#define OCTAVES 11
static float scan_note(SGSParser *o) {
...
static bool scan_label(SGSParser *o, LabelBuf label, char op) {
  uint32_t i = 0;
  char nolabel_msg[] = "ignoring ? without label name";
  nolabel_msg[9] = op; /* replace ? */
  for (;;) {
    o->c = SGS_READER_GETC(&o->fr);
    if (IS_WHITESPACE(o->c) || o->c == 0) {
      SGS_READER_UNGETC(&o->fr);
      if (i == 0)
...
static int32_t scan_wavetype(SGSParser *o) {
  ...
  int32_t wave = read_strfind(&o->fr, wavetypes);
  if (wave < 0)
    warning(o, "invalid wave type follows; sin, sqr, tri, saw available");
  return wave;
}

static bool scan_valit(SGSParser *o, float (*scan_symbol)(SGSParser *o),
                        SGSProgramValit *vi) {
  ...
  while ((c = scan_char(o)) != 0) {
    ...
    case NEWLINE:
      ...
      break;
    case 'c':
      type = read_strfind(&o->fr, valittypes);
      if (type >= 0) {
...
static bool scan_waittime(NodeScope *ns) {
  SGSParser *o = ns->o;
  /* FIXME: ADD_WAIT_DURATION */
  if (SGS_READER_TESTCGET(&o->fr, 't')) {
    if (!ns->last_operator) {
...
static void destroy_operator(SGSOperatorNode *op) {
  ...
  size_t i;
  struct SGSOperatorNode **ops;
  ops = (struct SGSOperatorNode**) SGS_PTRARR_ITEMS(&op->fmods);
  for (i = op->fmods.copy_count; i < op->fmods.count; ++i) {
    destroy_operator(ops[i]);
  }
  ops = (struct SGSOperatorNode**) SGS_PTRARR_ITEMS(&op->pmods);
  for (i = op->pmods.copy_count; i < op->pmods.count; ++i) {
    destroy_operator(ops[i]);
  }
  ops = (struct SGSOperatorNode**) SGS_PTRARR_ITEMS(&op->amods);
...
void SGS_event_node_destroy(SGSEventNode *e) {
  size_t i;
  struct SGSOperatorNode **ops;
  ops = (struct SGSOperatorNode**) SGS_PTRARR_ITEMS(&e->operators);
...
static bool parse_step(NodeScope *ns) {
  ...
  while ((c = scan_char(o)) != 0) {
    ...
    case 'a':
      ...
      } else if (SGS_READER_TESTCGET(&o->fr, '[')) {
    ...
    case 't':
      if (SGS_READER_TESTCGET(&o->fr, '*')) {
        ...
        op->time_ms = o->def_time_ms;
      } else if (SGS_READER_TESTCGET(&o->fr, 'i')) {
        if (!(ns->ns_flags & NS_NESTED_SCOPE)) {
...
/**
 * Process file and return result. The result needs to be freed
 * once no longer used.
 */
struct SGSParseList *SGS_parser_process(SGSParser *o, const char *filename) {
	...
	/* TODO: better return type management */
	struct SGSParseList *ret = calloc(1, sizeof(struct SGSParseList));
	ret->events = o->events;
	return ret;
}

/**
 * Parse a file and return result. The result needs to be freed
 * once no longer used.
 */
struct SGSParseList* SGS_parse(const char *filename) {
	SGSParser *o = SGS_create_parser();
	struct SGSParseList *result = SGS_parser_process(o, filename);
	SGS_destroy_parser(o);
	return result;
}
...
static void time_event(SGSEventNode *e) {
  ...
  if (e->composite) {
    ...
    SGSOperatorNode *ce_op = (struct SGSOperatorNode*)
    ...
    for (;;) {
      ...
      ce = ce->next;
      if (!ce) break;
      ce_op = (struct SGSOperatorNode*) SGS_PTRARR_GET(&ce->operators, 0);
    }
  }
}
