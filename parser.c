...
struct SGSParser {
  ...
  /* results */
  struct SGSParseResult *results;
  struct SGSParseResult *last_result;
};

/*
 * Set default values, used until changed by a script.
 */
static void set_defaults(SGSParser_t o) {
	o->ampmult = 1.f;
	o->def_time_ms = 1000;
	o->def_freq = 444.f;
...
SGSParser_t SGS_create_parser(void) {
	SGSParser_t o = calloc(1, sizeof(struct SGSParser));
	set_defaults(o);
	return o;
}
...
#define NEWLINE '\n'
static char read_char(SGSParser_t o) {
  char c;
  eatws(o->f);
  if (o->nextc) {
    c = o->nextc;
...
static void read_ws(SGSParser_t o) {
  char c;
  do {
    c = getc(o->f);
    if (c == ' ' || c == '\t')
...
static float read_num_r(SGSParser_t o, float (*read_symbol)(SGSParser_t o),
...
static bool read_num(SGSParser_t o, float (*read_symbol)(SGSParser_t o),
                      float *var) {
  char buf[64];
 ...
 */
static void warning(SGSParser_t o, const char *str) {
  char buf[4] = {'\'', o->c, '\'', 0};
...
#define OCTAVES 11
static float read_note(SGSParser_t o) {
...
static bool read_label(SGSParser_t o, LabelBuf label, char op) {
  uint32_t i = 0;
...
static int32_t read_wavetype(SGSParser_t o) {
...
static bool read_valit(SGSParser_t o, float (*read_symbol)(SGSParser_t o),
                        SGSProgramValit_t *vi) {
...
static void begin_scope(SGSParser_t o, NodeScope *ns, NodeScope *parent,
...
static bool parse_level(SGSParser_t o, NodeScope *parentnd,
                        uint8_t linktype, char newscope);
...
static bool parse_level(SGSParser_t o, NodeScope *parentns,
                        uint8_t linktype, char newscope) {
  char c;
  bool endscope = false;
  uint8_t flags = 0;
...
static void postparse_passes(SGSParser_t o);

/*
 * Update parser results, adding the result for the last file.
 */
static void add_result(SGSParser_t o) {
	struct SGSParseResult *result = calloc(1,
		sizeof(struct SGSParseResult));
	SGSEventNode *events = o->events;
	result->events = events;

	if (!o->results) o->results = result;
	if (o->last_result) {
...
/**
 * Clear state of parser. Destroy symbol table, set default values,
 * and clear results.
 */
void SGS_parser_clear(SGSParser_t o) {
	...
	struct SGSParseResult *result = o->results;
	while (result) {
		struct SGSParseResult *next = result->next;
		free(result);
		result = next;
	}
	o->results = NULL;
