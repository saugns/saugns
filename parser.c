 ...
 */
void SGS_nodelist_add(SGSNodeList *list, void *n) {
  SGSOperatorNode **nl;
  ++list->count;
  if (list->count <= 2) {
    if (list->count == 1) {
 ...
 * of the list will remain "inactive" - SGS_nodelist_operator_rforeach()
 * as well as the cleanup code of the owner of the list will ignore them,
 ...
 */
static int32_t SGS_nodelist_operator_rforeach(SGSNodeList *list,
  ...
  for (i = list->inactive_count; i < list->count; ++i) {
    ...
    ret += callback(op, arg);
    ret += SGS_nodelist_operator_rforeach(&op->fmods, callback, arg);
    ret += SGS_nodelist_operator_rforeach(&op->pmods, callback, arg);
    ret += SGS_nodelist_operator_rforeach(&op->amods, callback, arg);
  }
  return ret;
}
 ...
 */
static void SGS_nodelist_operator_rcleanup(SGSNodeList *list) {
  SGSOperatorNode **nl = SGS_NODELIST_GET(list);
  uint32_t i;
  for (i = list->inactive_count; i < list->count; ++i) {
    SGSOperatorNode *op = nl[i];
    SGS_nodelist_clear(&op->on_next);
    SGS_nodelist_operator_rcleanup(&op->fmods);
    SGS_nodelist_operator_rcleanup(&op->pmods);
    SGS_nodelist_operator_rcleanup(&op->amods);
...
void SGS_event_node_destroy(SGSEventNode *e) {
  SGS_nodelist_operator_rcleanup(&e->operators);
...
static void time_event(SGSEventNode *e) {
  ...
  SGS_nodelist_operator_rforeach(&e->operators, time_operator, 0);
  /*
