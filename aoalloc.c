...
SGS_AOAlloc_t SGS_create_aoalloc(size_t block_size) {
	SGS_AOAlloc_t o = calloc(1, sizeof(struct SGS_AOAlloc));
 ...
 */
static ptrdiff_t first_smallest_block(SGS_AOAlloc_t o, size_t size) {
	size_t i = 0;
	ptrdiff_t min = 0;
 ...
 * the block at \p to.
 */
static void copy_blocks_up_one(SGS_AOAlloc_t o, size_t to, size_t from) {
	if (from + 1 == to ||
 ...
 */
static ptrdiff_t extend_mempool(SGS_AOAlloc_t o) {
	size_t new_block_alloc = (o->block_alloc > 0) ?
		(o->block_alloc << 1) :
		1;
	o->blocks = realloc(o->blocks, sizeof(struct BlockEntry) * new_block_alloc);
	if (o->blocks == NULL)
		return -1;
...
/**
 * Allocate object of \p size within the memory pool.
 *
...
void *SGS_aoalloc_alloc(SGS_AOAlloc_t o, size_t size) {
	...
	if (i > 0) {
		...
		if (j < (size_t)i) {
			 ...
			 */
			struct BlockEntry tmp = o->blocks[i];
			o->blocks[i].free = o->blocks[i - 1].free;
...
/**
 * Allocate object of \p size within the memory pool, copying from \p mem
 * if not NULL.
 *
 * \return the allocated object, or NULL if allocation fails
 */
void *SGS_aoalloc_dup(SGS_AOAlloc_t o, const void *mem, size_t size) {
	void *ret = SGS_aoalloc_alloc(o, size);
	if (ret == NULL) return NULL;
	if (mem != NULL) memcpy(ret, mem, size);
	return ret;
}
