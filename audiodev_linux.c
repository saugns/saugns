 ...
 */
static inline SGSAudioDev_t open_audiodev_linux(const char *alsa_name,
	...
	SGSAudioDev_t o = malloc(sizeof(struct SGSAudioDev));
	o->ref.handle = handle;
	o->type = TYPE_ALSA;
	o->channels = channels;
	o->srate = *srate;
 ...
 */
static inline void close_audiodev_linux(SGSAudioDev_t o) {
	if (o->type == TYPE_OSS) {
 ...
 */
static inline bool audiodev_linux_write(SGSAudioDev_t o, const int16_t *buf,
		uint32_t samples) {
	if (o->type == TYPE_OSS)
