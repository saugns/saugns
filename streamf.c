...
static void file_ref_close(SGS_Stream *o);
...
bool SGS_Stream_fopenrb(SGS_Stream *o, const char *fname) {
	...
	o->io_ref = f;
	o->close_ref = file_ref_close;
	o->name = fname;
	o->active = SGS_STREAM_OPEN_R;
 ...
 * status set to either SGS_STREAM_END or SGS_STREAM_ERROR. If the file
...
static size_t file_mode_fread(SGS_CBufMode *o) {
	SGS_Stream *fo = (SGS_Stream*) o->ref;
	FILE *f = fo->io_ref;
	size_t len;
	/*
	...
	if (ferror(f)) {
		fo->status = SGS_STREAM_ERROR;
		file_ref_close(fo);
	} else if (feof(f)) {
		fo->status = SGS_STREAM_END;
		file_ref_close(fo);
	}
	return len;
}
 ...
 * Close file.
 */
void file_ref_close(SGS_Stream *o) {
	if (o->io_ref) {
		fclose((FILE*) o->io_ref);
		o->io_ref = NULL;
	}
}
