 ...
 */
static uint32_t calc_bufs(SGS_Generator_t o, struct OperatorNode *n) {
 ...
 * TODO: Verify, remove debug printing when parser module done.
 */
static void upsize_bufs(SGS_Generator_t o, struct VoiceNode *vn) {
  uint32_t count = 0, i, res;
  if (!vn->graph) return;
 ...
 */
static void handle_event(SGS_Generator_t o, struct EventNode *e) {
 ...
 */
static bool run_param(union BufData *buf, uint32_t buf_len, struct ParameterValit *vi,
                       float *state, const union BufData *modbuf) {
 ...
 */
static uint32_t run_block(SGS_Generator_t o, Buf_t *bufs, uint32_t buf_len,
  ...
  if (!wave_env) {
     ...
     * input.
     */
    const SGS_WaveLUTPtr_t lut = SGS_waveluts[n->wave];
    for (i = 0; i < len; ++i) {
      int32_t s, spm = 0;
 ...
 */
static uint32_t run_voice(SGS_Generator_t o, struct VoiceNode *vn, int16_t *out,
                      uint32_t buf_len) {
  uint32_t out_len = 0;
  bool finished = true;
