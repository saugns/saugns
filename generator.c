 ...
 */
static uint32_t calc_bufs(SGSGenerator_t o, OperatorNode *n) {
 ...
 * TODO: Verify, remove debug printing when parser module done.
 */
static void upsize_bufs(SGSGenerator_t o, VoiceNode *vn) {
  uint32_t count = 0, i, res;
  if (!vn->graph) return;
...
SGSGenerator_t SGS_create_generator(SGSProgram_t prg, uint32_t srate) {
  SGSGenerator_t o;
  SGSProgramEvent_t step;
  void *data;
  size_t i;
  /*
   * Establish allocation sizes.
   */
  size_t size = sizeof(struct SGSGenerator) - sizeof(OperatorNode);
 ...
 */
static void handle_event(SGSGenerator_t o, EventNode *e) {
 ...
 */
static uint32_t run_block(SGSGenerator_t o, Buf *bufs, uint32_t buf_len,
 ...
 */
static uint32_t run_voice(SGSGenerator_t o, VoiceNode *vn, int16_t *out,
                      uint32_t buf_len) {
  uint32_t ret_len = 0;
  bool finished = true;
