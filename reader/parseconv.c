...
create_ProgramOpList(const SAU_ScriptListData *restrict list_in,
	...
	if (!count)
		return &blank_oplist;
	SAU_ProgramOpList *o = SAU_mpalloc(mem,
...
ParseConv_convert_opdata(ParseConv *restrict o,
	...
	if (mods[SAU_POP_FPMOD] != NULL) {
		if (!set_oplist(&oas->fpmods, mods[SAU_POP_FPMOD], o->mem))
			goto MEM_ERR;
...
ParseConv_convert_event(ParseConv *restrict o,
	...
	SAU_ProgramEvent *out_ev = SAU_mpalloc(o->mem,
	...
	if ((vas->flags & SAU_VAS_GRAPH) != 0) {
		SAU_ProgramVoData *ovd = SAU_mpalloc(o->mem,
...
ParseConv_create_program(ParseConv *restrict o,
		SAU_Script *restrict script) {
	SAU_Program *prg = SAU_mpalloc(o->mem, sizeof(SAU_Program));
	if (!prg) goto MEM_ERR;
...
ParseConv_convert(ParseConv *restrict o,
	...
	o->mem = script->code_mem; // won't be cleared while script/parse kept
...
SAU_build_Program(SAU_Script *restrict parse, bool keep_parse) {
	...
	if (!keep_parse) {
		if (o) { parse->code_mem = NULL; o->parse = NULL; }
...
SAU_discard_Program(SAU_Program *restrict o) {
	if (!o)
		return;
	if (o->parse && o->parse->code_mem == o->mem) // avoid double-destroy
		o->parse->code_mem = NULL;
