...
static void program_convert_onode(ProgramAlloc *pa, SGS_ScriptOpData *op,
  ...
  SGS_ProgramEvent *out_ev = oa->out;
  ...
  if (op->amods)
    ood->amods=oa->amods = SGS_create_ProgramIDArr(pa->mp, op->amods,oa->amods);
  if (op->fmods)
    ood->fmods=oa->fmods = SGS_create_ProgramIDArr(pa->mp, op->fmods,oa->fmods);
  if (op->pmods)
    ood->pmods=oa->pmods = SGS_create_ProgramIDArr(pa->mp, op->pmods,oa->pmods);
}
...
static void program_convert_enode(ProgramAlloc *pa, SGS_ScriptEvData *e) {
  ...
  if (e->voice_params != 0) {
    ...
    if ((e->voice_params & SGS_P_GRAPH) != 0) {
      ovd->graph = SGS_create_ProgramIDArr(pa->mp, &e->graph, NULL);
    }
  }
}
