 ...
 */
static inline void VoAlloc_clear(VoAlloc *restrict o) {
	_VoAlloc_clear(o);
}
 ...
 */
static inline void OpAlloc_clear(OpAlloc *restrict o) {
	_OpAlloc_clear(o);
}
...
static void ParseConv_convert_opdata(ParseConv *restrict o,
	...
	if (op->amods) {
		ood->amods = oas->amods =
			SGS_create_ProgramIDArr(o->mp, op->amods, oas->amods);
		vas->flags |= VA_OPLIST;
	}
	if (op->fmods) {
		ood->fmods = oas->fmods =
			SGS_create_ProgramIDArr(o->mp, op->fmods, oas->fmods);
		vas->flags |= VA_OPLIST;
	}
	if (op->pmods) {
		ood->pmods = oas->pmods =
			SGS_create_ProgramIDArr(o->mp, op->pmods, oas->pmods);
		vas->flags |= VA_OPLIST;
	}
}
