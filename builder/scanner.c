 ...
 */
uint8_t SAU_Scanner_filter_space_keep(SAU_Scanner *restrict o SAU__maybe_unused,
 ...
 *
 * \return 0 or SAU_SCAN_SPACE
 */
uint8_t SAU_Scanner_filter_space_join(SAU_Scanner *restrict o,
		uint8_t c SAU__maybe_unused) {
	SAU_File *f = o->f;
	o->sf.char_num += SAU_File_skipspace(f);
	if (o->sf.c_flags & SAU_SCAN_C_SPACE)
		return 0;
	o->sf.c_flags |= SAU_SCAN_C_SPACE;
	return SAU_SCAN_SPACE;
}
 ...
 *
 * \return 0 or SAU_SCAN_LNBRK
 */
uint8_t SAU_Scanner_filter_linebreak_join(SAU_Scanner *restrict o, uint8_t c) {
	SAU_File *f = o->f;
	if (c == '\n') SAU_File_TRYC(f, '\r');
	while (SAU_File_trynewline(f)) pos_past_linebreak(o, 0);
	if (o->sf.c_flags & SAU_SCAN_C_LNBRK) {
...
/**
 * Reduce spaces to one SAU_SCAN_SPACE,
 * and a sequence of spaces and/or linebreaks to one SAU_SCAN_LNBRK.
 *
 * \return 0, SAU_SCAN_SPACE, or SAU_SCAN_LNBRK
 */
uint8_t SAU_Scanner_filter_ws_join(SAU_Scanner *restrict o, uint8_t c) {
	SAU_File *f = o->f;
	if (o->sf.c_flags & SAU_SCAN_C_LNBRK)
		return SAU_Scanner_filter_ws_none(o, c);
	if (c == '\n') {
		SAU_File_TRYC(f, '\r');
	} else if (c != '\r') {
		o->sf.char_num += SAU_File_skipspace(f);
...
LNBRK:
	...
	if (space_count > 0) {
		o->sf.c_flags &= ~SAU_SCAN_C_LNBRK_POSUP;
		pos_past_linebreak(o, space_count);
		goto LNBRK;
	}
	return SAU_SCAN_LNBRK;
}
...
uint8_t SAU_Scanner_setws_level(SAU_Scanner *restrict o, uint8_t ws_level) {
	...
	case SAU_SCAN_WS_ALL: // default level
		...
		o->filters[' ']  = SAU_Scanner_filter_space_keep;
		break;
	case SAU_SCAN_WS_RED1:
		o->filters['\t'] = SAU_Scanner_filter_space_join;
		o->filters['\n'] = SAU_Scanner_filter_linebreak_join;
		o->filters['\r'] = SAU_Scanner_filter_linebreak_join;
		o->filters[' ']  = SAU_Scanner_filter_space_join;
		break;
	case SAU_SCAN_WS_RED2:
		o->filters['\t'] = SAU_Scanner_filter_ws_join;
		o->filters['\n'] = SAU_Scanner_filter_ws_join;
		o->filters['\r'] = SAU_Scanner_filter_ws_join;
		o->filters[' ']  = SAU_Scanner_filter_ws_join;
		break;
