 ...
 */
SGS_Scanner *SGS_create_Scanner(SGS_SymTab *symtab) {
	if (!symtab) return NULL;

	SGS_Scanner *o = calloc(1, sizeof(SGS_Scanner));
	if (!SGS_init_File(&o->f)) goto ERROR;
	o->symtab = symtab;
 ...
 */
bool SGS_Scanner_fopenrb(SGS_Scanner *o, const char *path) {
	if (!SGS_File_fopenrb(&o->f, path)) return false;
 ...
 * \return 0
 */
uint8_t SGS_Scanner_handle_space(SGS_Scanner *o, uint8_t c SGS__maybe_unused) {
...
uint8_t SGS_Scanner_handle_linecomment(SGS_Scanner *o,
	...
	o->char_pos += SGS_File_skipline(&o->f);
	return 0;
}
 ...
 * \return \p c or 0
 */
uint8_t SGS_Scanner_handle_char1comments(SGS_Scanner *o, uint8_t c) {
 ...
 */
uint8_t SGS_Scanner_getc(SGS_Scanner *o) {
	SGS_CBuf *cb = &o->f.cb;
	uint8_t c;
 ...
 */
bool SGS_Scanner_tryc(SGS_Scanner *o, uint8_t testc) {
 ...
 */
bool SGS_Scanner_getsymstr(SGS_Scanner *o, const void **strp, uint32_t *lenp) {
	SGS_CBuf *cb = &o->f.cb;
	uint32_t len;
	bool truncated;
	SGS_CBufMode_DECP(&cb->r);
