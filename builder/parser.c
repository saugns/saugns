...
static bool init_ScanLookup(ScanLookup *restrict o, SGS_SymTab *restrict st) {
	...
	if (!o->wave_names)
		return false;
	o->slope_names = SGS_SymTab_pool_stra(st,
...
static bool scan_slp_state(SGS_Scanner *restrict o, NumSym_f scan_numsym,
...
static bool scan_slp_slope(SGS_Scanner *restrict o, NumSym_f scan_numsym,
		SGS_Slope *restrict slp, bool ratio) {
	ScanLookup *sl = o->data;
	bool goal = false;
	float vt;
...
static void begin_event(ParseLevel *restrict pl, uint8_t linktype,
	...
	if (pl->on_prev != NULL) {
		...
		if (pve->composite != NULL && !is_composite) {
			SGS_ScriptEvData *last_ce = pve->composite;
	...
	if (e->composite != NULL) {
		...
		for (;;) {
			...
			if ((ce_op->op_flags & SGS_SDOP_TIME_DEFAULT) != 0) {
				...
				ce_op->time_ms =
((ce_op->op_flags & SGS_SDOP_NESTED) != 0 && !ce->next) ?
					SGS_TIME_INF :
