...
static bool read_num(SGS_Parser *o, NumSym_f read_numsym,
  ...
  NumParser np = {o, read_numsym, {0}, false};
...
static bool read_ramp(SGS_Parser *o, NumSym_f read_numsym,
  ...
  while ((c = read_char(o)) != EOF) {
    ...
    case 't': {
      ...
      break; }
    case 'v':
      if (read_num(o, read_numsym, &ramp->goal, mul_inv))
        goal = true;
      break;
    case '}':
      goto RETURN;
...
static void begin_operator(ParseLevel *pl, uint8_t linktype,
  ...
  if (pop != NULL) {
     ...
     */
    op->time.v_ms = o->sopt.def_time_ms;
    op->amp = 1.0f;
  ...
  while ((c = read_char(o)) != EOF) {
    ...
    case 't':
      if (tryc('*', o->f)) {
        ...
        op->time.flags = SGS_TIMEP_SET | SGS_TIMEP_LINKED;
      } else {
        float time;
        ...
        op->time.v_ms = lrint(time * 1000.f);
        op->time.flags = SGS_TIMEP_SET;
      }
...
static void flatten_events(SGS_ScriptEvData *e) {
  ...
  while (ne != NULL) {
    ...
    if (fe->wait_ms >= ne->wait_ms) {
      ...
      SGS_ScriptEvData *fe_next = ne->next;
      fe->next = ne;
      ne->next = fe_next;
      fe = fe_next;
      if (fe)
