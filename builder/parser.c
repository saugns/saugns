...
static bool read_num(SGS_Parser *o, NumSym_f read_numsym,
                     float *var, bool mul_inv) {
  NumParser np = {o, read_numsym, {0}};
  float num = read_num_r(&np, 0, 0);
  if (num != num)
    return false;
  if (mul_inv) num = 1.f / num;
  if (fabs(num) == INFINITY) {
    warning(o, "discarding infinite number");
    return false;
  }
  *var = num;
  return true;
...
static bool read_slope(SGS_Parser *o, NumSym_f read_numsym,
  ...
  while ((c = read_char(o)) != EOF) {
    ...
    case 'c':
      ...
      type = strfind(o->f, SGS_Slope_names + 1) + 1;
      if (type <= 0) {
        warning(o, "invalid slope change type; available types are:");
        uint8_t i = 1;
    ...
    case 't': {
      ...
      break; }
    case 'v':
      if (read_num(o, read_numsym, &slope->goal, mul_inv))
        goal = true;
      break;
    case '}':
      goto RETURN;
...
RETURN:
  if (!goal) {
    warning(o, "ignoring value slope with no target value");
...
static bool parse_settings(ParseLevel *pl) {
  ...
  while ((c = read_char(o)) != EOF) {
    switch (c) {
    case 'a':
      if (read_num(o, 0, &o->sopt.ampmult, false)) {
        o->sopt.changed |= SGS_SOPT_AMPMULT;
      }
      break;
    case 'f':
      if (read_num(o, read_note, &o->sopt.def_freq, false)) {
        o->sopt.changed |= SGS_SOPT_DEF_FREQ;
      }
      break;
    case 'n': {
      float freq;
      if (read_num(o, 0, &freq, false)) {
        if (freq < 1.f) {
      ...
      break; }
    case 'r':
      if (read_num(o, 0, &o->sopt.def_ratio, true)) {
  ...
  while ((c = read_char(o)) != EOF) {
    ...
    case 'P':
      if ((pl->pl_flags & SDPL_NESTED_SCOPE) != 0)
        goto UNKNOWN;
      if (tryc('{', o->f)) {
        if (read_slope(o, 0, &e->slope_pan, false))
          e->vo_attr |= SGS_PVOA_SLOPE_PAN;
      } else if (read_num(o, 0, &e->pan, false)) {
        if (e->slope_pan.type == SGS_SLOPE_STATE)
          e->vo_attr &= ~SGS_PVOA_SLOPE_PAN;
      }
      break;
    ...
    case 'a':
      if (tryc('!', o->f)) {
        ...
        if (read_slope(o, 0, &op->slope_amp, false))
          op->attr |= SGS_POPA_SLOPE_AMP;
      } else {
        read_num(o, 0, &op->amp, false);
    ...
    case 'f':
      if (tryc('!', o->f)) {
        if (!testc('[', o->f)) {
          if (read_num(o, 0, &op->dynfreq, false)) {
        ...
        if (read_slope(o, read_note, &op->slope_freq, false)) {
          op->attr |= SGS_POPA_SLOPE_FREQ;
          op->attr &= ~SGS_POPA_SLOPE_FREQRATIO;
        }
      } else if (read_num(o, read_note, &op->freq, false)) {
        op->attr &= ~SGS_POPA_FREQRATIO;
    ...
    case 'p':
      if (tryc('+', o->f)) {
        if (tryc('[', o->f)) {
          ...
          goto UNKNOWN;
      } else if (read_num(o, 0, &op->phase, false)) {
    ...
    case 'r':
      ...
      if (tryc('!', o->f)) {
        if (!testc('[', o->f)) {
          if (read_num(o, 0, &op->dynfreq, true)) {
            op->attr |= SGS_POPA_DYNFREQRATIO;
          }
        }
        if (tryc('[', o->f)) {
        ...
        if (read_slope(o, read_note, &op->slope_freq, true)) {
          op->attr |= SGS_POPA_SLOPE_FREQ |
                      SGS_POPA_SLOPE_FREQRATIO;
        }
      } else if (read_num(o, 0, &op->freq, true)) {
        op->attr |= SGS_POPA_FREQRATIO;
        op->op_params |= SGS_POPP_FREQ;
        if (op->slope_freq.type == SGS_SLOPE_STATE)
          op->attr &= ~(SGS_POPA_SLOPE_FREQ |
