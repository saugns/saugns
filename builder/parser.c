...
static bool SGS__noinline scan_num(PScanner *restrict o,
...
static void parse_level(SGS_Parser *restrict o,
                        ParseLevel *restrict parent_pl,
                        uint8_t linktype, uint8_t newscope);
    ...
    case 'p':
      if (SGS_File_TRYC(f, '+')) {
        if (SGS_File_TRYC(f, '[')) {
          ...
          goto UNKNOWN;
      } else if (scan_num(sc, NULL, &op->phase)) {
...
static void parse_level(SGS_Parser *restrict o,
  ...
  for (;;) {
    ...
    case 'S':
      ...
      break;
    case '[':
      parse_level(o, &pl, pl.linktype, SCOPE_NEST);
      break;
    case '\\':
