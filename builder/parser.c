...
static void init_scanner(PScanner *restrict o,
                         ...
                         SGS_ScriptOptions *restrict sopt) {
  o->f = SGS_create_File();
  o->st = st;
  o->sopt = sopt;
}
...
static bool scan_fopenrb(PScanner *restrict o, const char *restrict path) {
  SGS_File *f = o->f;
  if (!SGS_File_fopenrb(f, path)) {
    SGS_error(NULL, "couldn't open script file \"%s\" for reading", path);
    return false;
  }
  o->line = 1;
  return true;
...
void scan_close(PScanner *restrict o) {
  SGS_File *f = o->f;
  SGS_File_close(f);
}
...
typedef float (*NumSym_f)(PScanner *o);
...
static size_t scan_label(PScanner *restrict o, LabelBuf label, char op) {
  ...
  bool truncated = !SGS_File_gets(f, label, LABEL_BUFLEN, &len, filter_symchar);
  if (len == 0) {
  ...
  if (truncated) {
    ...
    SGS_File_skips(f, filter_symchar);
  }
  o->c = SGS_File_RETC(f);
  return len;
}

static int32_t scan_wavetype(PScanner *restrict o) {
  SGS_File *f = o->f;
...
static bool scan_slope(PScanner *restrict o, NumSym_f scan_numsym,
                       SGS_TimedParam *restrict tpar, bool mul_inv) {
  SGS_File *f = o->f;
  bool goal = false;
...
static bool parse_settings(ParseLevel *restrict pl) {
  ...
  for (;;) {
    ...
    switch (c) {
    case 'a':
      if (scan_num(sc, 0, &o->sopt.ampmult, false)) {
        o->sopt.changed |= SGS_SOPT_AMPMULT;
      }
      break;
    case 'f':
      if (scan_num(sc, scan_note, &o->sopt.def_freq, false)) {
    ...
    case 'n': {
      ...
      break; }
    case 'r':
      if (scan_num(sc, 0, &o->sopt.def_ratio, true)) {
