...
static bool init_ScanLookup(ScanLookup *restrict o, SAU_SymTab *restrict st) {
	...
	if (!o->wave_names)
		return false;
	o->slope_names = SAU_SymTab_pool_stra(st,
			SAU_Slope_names, SAU_SLOPE_TYPES);
	if (!o->slope_names)
		return false;
	return true;
}
...
static bool scan_slp_state(SAU_Scanner *restrict o, NumSym_f scan_numsym,
		SAU_Slope *restrict slp, bool ratio) {
	if (!scan_num(o, scan_numsym, &slp->v0, ratio))
		return false;
	if (ratio) {
		slp->flags |= SAU_SLP_STATE_RATIO;
	} else {
		slp->flags &= ~SAU_SLP_STATE_RATIO;
	}
	slp->flags |= SAU_SLP_STATE;
	return true;
}

static bool scan_slp_slope(SAU_Scanner *restrict o, NumSym_f scan_numsym,
		SAU_Slope *restrict slp, bool ratio) {
	ScanLookup *sl = o->data;
	bool goal = false;
	float vt;
	...
	if ((slp->flags & SAU_SLP_SLOPE) != 0) {
		// allow partial change
		if (((slp->flags & SAU_SLP_SLOPE_RATIO) != 0) == ratio) {
			goal = true;
			vt = slp->vt;
		}
		time_ms = slp->time_ms;
	...
	for (;;) {
		...
		case 'c': {
			size_t type;
			if (scan_symafind(o, sl->slope_names, SAU_SLOPE_TYPES,
					&type, "slope change type")) {
				slope = type;
			}
			break; }
		case 't': {
...
RETURN:
	...
	slp->slope = slope;
	if (ratio) {
		slp->flags |= SAU_SLP_SLOPE_RATIO;
	} else {
		slp->flags &= ~SAU_SLP_SLOPE_RATIO;
	}
	slp->flags |= SAU_SLP_SLOPE;
	return true;
}
...
static void end_operator(ParseLevel *restrict pl) {
	...
	if (SAU_Slope_ENABLED(&op->freq))
		op->op_params |= SAU_POPP_FREQ;
	if (SAU_Slope_ENABLED(&op->amp)) {
...
static void begin_operator(ParseLevel *restrict pl, uint8_t linktype,
	 ...
	 * Initialize node.
	 */
	SAU_Slope_reset(&op->freq);
	SAU_Slope_reset(&op->amp);
	if (pop != NULL) {
		...
		if (!(pl->pl_flags & SDPL_NESTED_SCOPE)) {
			...
			op->freq.v0 = sl->sopt.def_ratio;
			op->freq.flags |= SAU_SLP_STATE_RATIO;
		}
		op->freq.flags |= SAU_SLP_STATE;
		op->amp.v0 = 1.0f;
		op->amp.flags |= SAU_SLP_STATE;
	}
	op->event = e;
	/*
...
"ignoring tuning frequency (Hz) below 1.0");
			...
			break; }
		case 'r':
			if (scan_num(sc, NULL, &sl->sopt.def_ratio, true))
