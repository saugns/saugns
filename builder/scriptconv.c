...
static uint32_t voice_duration(const SGS_ScriptEvData *restrict ve) {
	...
	ops = (SGS_ScriptOpData**) SGS_PtrList_ITEMS(&ve->op_graph);
...
//		SGS_OpAllocState *oas = &oa->a[id];
//		if (!(oas->last_sod->op_flags & SGS_SDOP_LATER_USED)
//			&& oas->duration_ms == 0) {
//			//free(oas->adjcs);
//			*oas = (SGS_OpAllocState){0};
//			*op_id = id;
//			return false;
//		}
//	}
...
static void ScriptConv_convert_ops(ScriptConv *restrict o,
	...
	for (size_t i = op_list->old_count; i < op_list->count; ++i) {
		...
		if (op->op_flags & SGS_SDOP_MULTIPLE) continue;
		uint32_t op_id = SGS_OpAlloc_update(&o->oa, op);
		ScriptConv_convert_opdata(o, op, op_id);
	}
	for (size_t i = 0; i < o->ev_op_data.count; ++i) {
		SGS_ProgramOpData *od = &o->ev_op_data.a[i];
		SGS_OpAllocState *oas = &o->oa.a[od->id];
		if (od->params & SGS_POPP_ADJCS) {
...
static SGS_Program *ScriptConv_create_program(ScriptConv *restrict o,
		SGS_Script *restrict script) {
	SGS_Program *prg = calloc(1, sizeof(SGS_Program));
	if (!prg) goto ERROR;
	if (!SGS_PtrList_memdup(&o->ev_list, (void***) &prg->events))
		goto ERROR;
	prg->ev_count = o->ev_list.count;
	if (!(script->sopt.changed & SGS_SOPT_AMPMULT)) {
		/*
 ...
 */
static SGS_Program *ScriptConv_convert(ScriptConv *restrict o,
		SGS_Script *restrict script) {
...
SGS_Program* SGS_build_Program(SGS_Script *restrict sd) {
	...
	SGS_Program *o = ScriptConv_convert(&sc, sd);
	return o;
}
