...
static uint32_t voice_duration(const SAU_ScriptEvData *restrict ve) {
	...
	ops = (SAU_ScriptOpData**) SAU_PtrList_ITEMS(&ve->operators);
...
static void print_opline(const SAU_ProgramOpData *restrict od) {
	...
	if ((od->freq.flags & SAU_TPAR_STATE) != 0) {
		if ((od->freq.flags & SAU_TPAR_SLOPE) != 0)
			fprintf(stdout,
	...
	if ((od->amp.flags & SAU_TPAR_STATE) != 0) {
		if ((od->amp.flags & SAU_TPAR_SLOPE) != 0)
			fprintf(stdout,
				"\ta=%-6.1f->%-6.1f", od->amp.v0, od->amp.vt);
		else
			fprintf(stdout,
				"\ta=%-6.1f", od->amp.v0);
	} else if ((od->amp.flags & SAU_TPAR_SLOPE) != 0) {
		fprintf(stdout,
			"\ta->%-6.1f", od->amp.vt);
	}
}
