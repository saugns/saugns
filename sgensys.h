/* sgensys: Common definitions.
 * Copyright (c) 2011-2012, 2018 Joel K. Pettersson
 * <joelkpettersson@gmail.com>.
 *
 * This file and the software of which it is part is distributed under the
 * terms of the GNU Lesser General Public License, either version 3 or (at
 * your option) any later version, WITHOUT ANY WARRANTY, not even of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
 *
 * View the file COPYING for details, or if missing, see
 * <https://www.gnu.org/licenses/>.
 */

#pragma once

/*
 * Common types.
 */

#include <stddef.h>
#include <stdint.h>
#include <stdbool.h>

typedef unsigned int uint;

/*
 * Debugging options.
 */

#define SGS_DEBUG_PRINT_PROGRAM 1

/* Disable old parser, run test code instead. */
#define SGS_TEST_SCANNER 0
#define SGS_TEST_LEXER 0

#define SGS_HASHTAB_STATS 0
#define SGS_LEXER_QUIET 0

/*
 * Keyword-like macros and compiler-dependent definitions.
 */

#if defined(__GNUC__) || defined(__clang__)
# define SGS__maybe_unused __attribute__((unused))
#else
# define SGS__maybe_unused
#endif

/* Use: Inline function provided whether or not it is ever used.
   May be needed to avoid warnings when the inline function is
   not in an included file, but rather generated by a macro in
   the source file using it. */
#define SGS__api_inline static inline SGS__maybe_unused
