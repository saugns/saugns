...
static size_t count_tpar_values(const SAU_TimedParam *restrict tpar) {
	size_t count = 1; // flags always included
	if ((tpar->flags & SAU_TPAR_STATE) != 0) ++count;
	if ((tpar->flags & SAU_TPAR_SLOPE) != 0) count += 3;
	return count;
}
...
static EventValue *convert_tpar_update(EventValue *restrict val,
		const SAU_TimedParam *restrict tpar) {
	uint8_t flags = tpar->flags;
	(*val++).i = flags;
	if ((flags & SAU_TPAR_STATE) != 0) {
		(*val++).f = tpar->v0;
	}
	if ((flags & SAU_TPAR_SLOPE) != 0) {
		(*val++).f = tpar->vt;
		(*val++).i = tpar->time_ms;
		(*val++).i = tpar->slope;
	}
	...
	for (size_t i = 0; i < prg->ev_count; ++i) {
		...
		for (size_t j = 0; j < prg_e->op_data_count; ++j) {
			...
			if (params & SAU_POPP_SILENCE)
				(*ev_v++).i = SAU_MS_TO_SRT(pod->silence_ms, srate);
 ...
 */
static const EventValue *handle_tpar_update(SAU_TimedParam *restrict tpar,
	...
	if (1) /* more types to be added in the future */ {
		...
		for (size_t i = 0; i < e->od_count; ++i) {
			...
			if (params & SAU_POPP_DYNFREQ)
				on->dynfreq = (*val++).f;
			if (params & SAU_POPP_PHASE)
				SAU_Osc_SET_PHASE(&on->osc, (uint32_t)(*val++).i);
			if (params & SAU_POPP_AMP)
...
static uint32_t run_block(SAU_Generator *restrict o,
	 ...
	 */
	freq = *(bufs++);
	SAU_TimedParam_run(&n->freq, freq, len, o->srate, &n->freq_pos, parent_freq);
	if (fmodc) {
		...
		fm_buf = *bufs;
		if ((n->freq.flags & SAU_TPAR_STATE_RATIO) != 0) {
			for (i = 0; i < len; ++i)
...
static void mix_add(SAU_Generator *restrict o,
	...
	float *mix_l = o->mix_bufs[0];
	float *mix_r = o->mix_bufs[1];
	if ((vn->pan.flags & SAU_TPAR_SLOPE) != 0) {
		float *pan_buf = o->gen_bufs[1];
		SAU_TimedParam_run(&vn->pan, pan_buf, len, o->srate, &vn->pan_pos, NULL);
...
PROCESS:
	...
	while (o->event < o->ev_count) {
		...
		if (o->event_pos < e->waittime) {
			 ...
			 * ensure events run before voices.
			 */
