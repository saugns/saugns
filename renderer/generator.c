...
enum {
  ON_VISITED = 1<<0,
  ON_TIME_INF = 1<<1, /* used for SGS_TIMEP_LINKED */
};
...
static bool alloc_for_program(SGS_Generator *restrict o,
  ...
  i = prg->ev_count;
  if (i > 0) {
    o->events = SGS_MemPool_alloc(o->mem, NULL, i * sizeof(EventNode*));
    if (!o->events) goto ERROR;
    o->ev_count = i;
  }
  i = prg->vo_count;
  if (i > 0) {
    o->voices = SGS_MemPool_alloc(o->mem, NULL, i * sizeof(VoiceNode));
    if (!o->voices) goto ERROR;
    o->vo_count = i;
  }
  i = prg->op_count;
  if (i > 0) {
    o->operators = SGS_MemPool_alloc(o->mem, NULL, i * sizeof(OperatorNode));
...
static bool convert_program(SGS_Generator *restrict o,
  ...
  for (size_t i = 0; i < prg->ev_count; ++i) {
    ...
    EventNode *e = SGS_MemPool_alloc(o->mem, NULL, sizeof(EventNode));
    if (!e)
      return false;
    uint32_t params;
    uint16_t vo_id = prg_e->vo_id;
    e->wait = SGS_MS_TO_SRT(prg_e->wait_ms, srate);
    vo_wait_time += e->wait;
...
SGS_Generator* SGS_create_Generator(SGS_Program *restrict prg,
  ...
  if (!mem)
    return NULL;
  SGS_Generator *o = SGS_MemPool_alloc(mem, NULL, sizeof(SGS_Generator));
  if (!o) {
    ...
    for (size_t i = 0; i < e->op_data_count; ++i) {
      ...
      if (params & SGS_POPP_TIME) {
        ...
        if (src->flags & SGS_TIMEP_LINKED) {
          ...
          on->time = SGS_MS_TO_SRT(src->v_ms, o->srate);
          on->flags &= ~ON_TIME_INF;
        }
      }
      if (params & SGS_POPP_SILENCE)
        on->silence = SGS_MS_TO_SRT(od->silence_ms, o->srate);
      if (params & SGS_POPP_FREQ)
        on->freq = od->freq;
