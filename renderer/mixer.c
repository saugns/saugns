 ...
 *
 * Sample rate needs to be set if \p pan has slope enabled.
 */
void SGS_Mixer_add(SGS_Mixer *restrict o,
		float *restrict buf, size_t len,
		SGS_Slope *restrict pan, uint32_t *restrict pan_pos) {
	if ((pan->flags & SGS_SLP_SLOPE) != 0) {
		SGS_Slope_run(pan, o->pan_buf, len, o->srate, pan_pos, NULL);
