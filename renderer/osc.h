 ...
 */
#define SSG_Osc_GET_PHASE(o) ((uint32_t) (o)->phase)

/**
 * Set phase as 32-bit unsigned int value.
 */
#define SSG_Osc_SET_PHASE(o, p) ((void) ((o)->phase = (p)))
 ...
 */
#define SSG_Osc_SRATE_COEFF(srate) ((double) 4294967296.0/(srate))
 ...
 */
#define SSG_Osc_CYCLE_LEN(coeff, freq) \
 ...
 * Can be used to adjust timing.
 */
#define SSG_Osc_CYCLE_POS(coeff, freq, spos, cpos_out) do{ \
	uint32_t SSG_Osc__inc = lrint((coeff)*(freq)); \
	uint32_t SSG_Osc__phs = SSG_Osc__inc * (uint32_t)(spos); \
	(cpos_out) = SSG_Osc__phs / SSG_Osc__inc; \
}while(0)

/**
 * Produce floating point output in the -1.0 to 1.0 range.
 */
static inline float SSG_Osc_run(SSG_Osc *o, const float *lut, double coeff,
		float freq, int16_t pm_s16) {
	uint32_t phase = o->phase + (pm_s16 << 16);
	float s = SSG_Wave_get_lerp(lut, phase);
