...
open to extending and reworking the SAU language. The constraints are
keeping the core of the language and its "spirit", and limiting the
computational complexity firstly, and code size secondly.

Where and how to report or suggest
----------------------------------

You can open an issue on Codeberg (or alternatively Framagit)
or send an email to <joelkp@tuta.io>.

Code by bots, copypasta, and extra dependencies are not welcome.
(Platform-specific dependencies to support the platform are fine.)
Bug reports must be verified by a person (if bugs were found by a
tool, this must be mentioned).

There's no further specific conduct rules yet, because there's so
little outside contribution there's yet to be any discussion on such.
...
Testing saugns using example scripts
------------------------------------
