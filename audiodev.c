 ...
 * written any number of times using SSG_AudioDev_write().
 *
...
#endif
	if (!o) {
		SSG_error(NULL, "couldn't open audio device for output");
		return NULL;
	}
	return o;
}
 ...
 */
void SSG_close_AudioDev(SSG_AudioDev *o) {
#ifdef __linux
	close_linux(o);
#elif defined(__OpenBSD__)
	close_sndio(o);
 ...
 */
uint32_t SSG_AudioDev_get_srate(const SSG_AudioDev *o) {
	return o->srate;
}
 ...
 */
bool SSG_AudioDev_write(SSG_AudioDev *o, const int16_t *buf, uint32_t samples) {
#ifdef __linux
