 ...
 */
#define SGSOsc_RUN_S16(o, lut, sr_coeff, freq, pm_s16, amp, s16_out) do{ \
	...
	(s16_out) = SGSOsc__s16; \
	/* update phase */ \
	uint32_t SGSOsc__inc = lrint((sr_coeff)*(freq)); \
	(o)->phase += SGSOsc__inc; \
}while(0)
 ...
 */
#define SGSOsc_RUN_SF(o, lut, sr_coeff, freq, pm_s16, sf_out) do{ \
	uint32_t SGSOsc__phs = (o)->phase + ((pm_s16) << 16); \
	uint32_t SGSOsc__ind = SGSOsc__phs >> (32-SGSOsc_LUT_INDEXBITS); \
	int32_t SGSOsc__s16 = (lut)[SGSOsc__ind]; \
	/* write lerp'd & scaled result */ \
